...
class Tbl_usuario {
    getAll() {
        ...
        getConnection().then((connection) => {
            connection.query('SELECT * FROM	tbl_parametro', (err, result) => {
            connection.release();
            if (err) {
                reject(err);
            } else {
    ...
    getById(itemId, callback) {
        console.log(itemId);
        getConnection().then((connection) => {
        connection.query('SELECT * FROM tbl_parametro WHERE I_IDPERFIL = ?', [itemId], (err, result) => {
            connection.release();
            callback(err, result);
        });
        }).catch((error) => {
    ...
    create(newItem, callback) {
        getConnection().then((connection) => {
        connection.query('INSERT INTO tbl_parametro SET ?', newItem, (err, result) => {
            connection.release();
            callback(err, result);
        });
        }).catch((error) => {
    ...
    delete(itemId, callback) {
        getConnection().then((connection) => {
        connection.query('DELETE FROM tbl_parametro WHERE I_IDPERFIL = ?', [itemId], (err, result) => {
            connection.release();
            callback(err, result);
        });
        }).catch((error) => {
